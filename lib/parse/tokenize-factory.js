...
function tokenizeFactory (parser, type) {
  ...
  function tokenize (value, location) {
    ...
    var tokens = []
    var methods = parser[type + 'Methods']
    ...
    function process () {
      ...
      function matchMethods (methods) {
        ...
        if (
          method &&
          (!method.onlyAtStart || parser.state.atStart) &&
          (!method.onlyAtTop || parser.state.atTop) &&
          (!method.notInBlockquote || !parser.state.inBlockquote) &&
          (!method.notInLink || !parser.state.inLink)
        ) {
          var valueLength = value.length

          return runAsync(method)(Object.assign({}, parser, {eat}), value)
            .then(() => {
