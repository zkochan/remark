...
function tokenizeList (parser, value, silent) {
  ...
  function tokenizeEach (index) {
    ...
    if (currentMarker && RULE_MARKERS[currentMarker] === true) {
      if (
        tokenizers.thematicBreak(parser, line, true)
      ) {
        return Promise.resolve()
      }
    }
    ...
    if (indented && item) {
      ...
      if (!pedantic) {
        return runAsync(tokenizers.fences)(parser, line, true)
          .then(found => {
            if (found) return

            return runAsync(tokenizers.thematicBreak)(parser, line, true)
              .then(found => {
                if (found) return
    ...
    function notCommonmarkNext () {
      if (!commonmark) {
        return runAsync(tokenizers.definition)(parser, line, true)
          .then(found => {
            if (found) return

            return runAsync(tokenizers.footnoteDefinition)(parser, line, true)
              .then(found => {
                if (found) return
